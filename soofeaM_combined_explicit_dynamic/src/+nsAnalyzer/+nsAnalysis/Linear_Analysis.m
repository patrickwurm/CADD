classdef Linear_Analysis < nsAnalyzer.nsAnalysis.Analysis

    methods
        function self = Linear_Analysis(model, output_handler, example_name)
            self@nsAnalyzer.nsAnalysis.Analysis(model, output_handler, example_name);
        end
        ...
        function paddisp = run(self, write_to_nodes, boundary_cond_matrix_interface, fe_step, ref_positions, positions, mxatm, mxpatm, updateDD, istep, settings)
            ...
            % Setzt die Einträge für DRBs in jedem Knoten, 'integriert'
            ...
            %obsolete?
            %self.model.bc_handler.incorporateBC(self.model, {});
            ...
            % Ich glaube nicht, dass dem Wert von 'unbalanced_energy' in
            % der linearen Analyse eine physikalische Bedeutung zukommt.
            self.solveFESystem;
%             disp(['unbalanced energy= ',num2str(unbalanced_energy)])
